...
async function fetchAdventures(city) {
  ...
  try {
    var res = await fetch(`${config.backendEndpoint}/adventures?city=${city}`);
...
function addAdventureToDOM(adventures) {
  ...
  adventures.forEach((element) => {
    let div = `<div class="col-6 col-lg-3 p-3 ">
...
function filterByDuration(list, low, high) {
  ...
  for (let i = 0; i < list.length; i++) {
    if (list[i].duration >= low && list[i].duration <= high) res.push(list[i]);
  }
  return res;
}
...
function filterByCategory(list, categoryList) {
  let res = [];
  for (let i = 0; i < list.length; i++) {
    if (categoryList.includes(list[i].category)) res.push(list[i]);
  }
  return res;
}
// TODO: MODULE_FILTERS
